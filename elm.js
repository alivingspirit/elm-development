...
Elm.SpaceGame.Player.make = function (_elm) {
   ...
   var update = F2(function (_p0,model) {
      var _p1 = _p0;
      var _p2 = A2($Utilities$Tuple.map,$Basics.toFloat,{ctor: "_Tuple2",_0: _p1._0,_1: _p1._1});
      var xmult$ = _p2._0;
      var ymult$ = _p2._1;
      var newDx = model.dx + xmult$ * velocityMultiplier;
      var newX = model.x + newDx;
      var newDy = model.dy + ymult$ * velocityMultiplier;
      var newY = model.y + newDy;
   ...
   return _elm.SpaceGame.Player.values = {_op: _op,Model: Model,init: init,velocityMultiplier: velocityMultiplier,update: update};
};
Elm.SpaceGame = Elm.SpaceGame || {};
...
Elm.SpaceGame.Main.make = function (_elm) {
   ...
   var update = F2(function (input,model) {
      var newPlayer = A2($SpaceGame$Player.update,{ctor: "_Tuple2",_0: input.keyboardX,_1: input.keyboardY},model.player);
      return _U.update(model,{player: newPlayer});
   });
   var Model = function (a) {    return {player: a};};
   var init = Model($SpaceGame$Player.init);
   var Input = F2(function (a,b) {    return {keyboardX: a,keyboardY: b};});
   var inputSignal = A2($Signal.map,function (key) {    return A2(Input,key.x,key.y);},$Keyboard.arrows);
   var timedInputSignal = A2($Signal.sampleOn,$Time.fps(35),inputSignal);
   var model = A3($Signal.foldp,update,init,timedInputSignal);
   var main = A2($Signal.map,$Graphics$Element.show,model);
