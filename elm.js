...
Elm.Utilities.Tuple = Elm.Utilities.Tuple || {};
Elm.Utilities.Tuple.make = function (_elm) {
   "use strict";
   _elm.Utilities = _elm.Utilities || {};
   _elm.Utilities.Tuple = _elm.Utilities.Tuple || {};
   if (_elm.Utilities.Tuple.values) return _elm.Utilities.Tuple.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var map = F2(function (fun,_p0) {    var _p1 = _p0;return {ctor: "_Tuple2",_0: fun(_p1._0),_1: fun(_p1._1)};});
   return _elm.Utilities.Tuple.values = {_op: _op,map: map};
};
Elm.SpaceGame = Elm.SpaceGame || {};
Elm.SpaceGame.Player = Elm.SpaceGame.Player || {};
Elm.SpaceGame.Player.make = function (_elm) {
   "use strict";
   _elm.SpaceGame = _elm.SpaceGame || {};
   _elm.SpaceGame.Player = _elm.SpaceGame.Player || {};
   ...
   var maxSpeed = 1.0;
   var update = F2(function (input,model) {
      var clamp$ = A2(clampFloat,0 - maxSpeed,maxSpeed);
      var _p0 = A2($Utilities$Tuple.map,$Basics.toFloat,{ctor: "_Tuple2",_0: input.x,_1: input.y});
      var xmult$ = _p0._0;
      var ymult$ = _p0._1;
      var newDx = model.dx + xmult$ * velocityMultiplier;
      var newX = clamp$(model.x + newDx);
      var newDy = model.dy + ymult$ * velocityMultiplier;
      var newY = clamp$(model.y + newDy);
      return _U.update(model,{x: newX,y: newY,dx: newDx,dy: newDy});
   });
   var Model = F4(function (a,b,c,d) {    return {x: a,y: b,dx: c,dy: d};});
   var init = A4(Model,0,0,0,0);
   return _elm.SpaceGame.Player.values = {_op: _op
                                         ,Model: Model
                                         ,maxSpeed: maxSpeed
                                         ,init: init
...
Elm.SpaceGame.Star.make = function (_elm) {
   "use strict";
   _elm.SpaceGame = _elm.SpaceGame || {};
   ...
   var calculateFromRange = F4(function (n,dn,speed,minmax) {
      var newN = n + dn * speed;
      return _U.cmp(0 - minmax,newN) < 0 && _U.cmp(newN,minmax) < 0 ? newN : _U.cmp(newN,0 - minmax) < 0 ? newN + minmax * 2 : newN - minmax * 2;
   });
   var update = F3(function (diff,window,model) {
      var windowY = window.y / 2 | 0;
      var newY = A4(calculateFromRange,model.y,diff.dy,$Basics.toFloat(model.speed),$Basics.toFloat(windowY));
      var windowX = window.x / 2 | 0;
      var newX = A4(calculateFromRange,model.x,diff.dx,$Basics.toFloat(model.speed),$Basics.toFloat(windowX));
      return _U.update(model,{x: newX,y: newY});
   });
   var Model = F5(function (a,b,c,d,e) {    return {x: a,y: b,size: c,speed: d,color: e};});
   return _elm.SpaceGame.Star.values = {_op: _op,Model: Model,calculateFromRange: calculateFromRange,update: update,view: view};
};
Elm.SpaceGame = Elm.SpaceGame || {};
...
Elm.SpaceGame.StarField.make = function (_elm) {
   ...
   var _op = {};
   var update = F3(function (diff,window,model) {
      var diff$ = {dx: 0 - diff.dx,dy: 0 - diff.dy};
      var newStars = A2($List.map,A2($SpaceGame$Star.update,diff$,window),model.stars);
      return _U.update(model,{stars: newStars});
   });
   ...
   var randomStarFromLayer = F2(function (windowSize,layer) {
      ...
      var randomSize = A2($Random.$float,layer.size.min,layer.size.max);
      var windowY = windowSize.y / 2 | 0;
      var randomY = A2($Random.$int,0 - windowY,windowY);
   ...
   var colorGeneratorFromList = function (list) {
      ...
      if (_p0.ctor === "[]") {
            ...
            {start: {line: 92,column: 3},end: {line: 110,column: 76}},
   ...
   var getLayerFromPercentage = F2(function (layers,i) {
      getLayerFromPercentage: while (true) {
         ...
         if (_p2.ctor === "[]") {
               ...
               var _p3 = _p2._0;
               var i$ = i - _p3.percent;
               if (_U.cmp(i$,0) < 1) return _p3; else {
                     var _v2 = _p2._1,_v3 = i$;
                     layers = _v2;
                     i = _v3;
                     continue getLayerFromPercentage;
                  }
            }
      }
   });
   ...
   var random = function (window) {    return A2($Random.map,Model,A2($Random.list,1000,randomStar(window)));};
...
Elm.Utilities = Elm.Utilities || {};
Elm.Utilities.Random = Elm.Utilities.Random || {};
Elm.Utilities.Random.make = function (_elm) {
   "use strict";
   _elm.Utilities = _elm.Utilities || {};
   _elm.Utilities.Random = _elm.Utilities.Random || {};
   if (_elm.Utilities.Random.values) return _elm.Utilities.Random.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
...
Elm.SpaceGame.Main.make = function (_elm) {
   "use strict";
   _elm.SpaceGame = _elm.SpaceGame || {};
   _elm.SpaceGame.Main = _elm.SpaceGame.Main || {};
   if (_elm.SpaceGame.Main.values) return _elm.SpaceGame.Main.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Collage = Elm.Graphics.Collage.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $SpaceGame$Player = Elm.SpaceGame.Player.make(_elm),
   $SpaceGame$StarField = Elm.SpaceGame.StarField.make(_elm),
   $Time = Elm.Time.make(_elm),
   $Utilities$Random = Elm.Utilities.Random.make(_elm);
   var _op = {};
   var window = {x: 1200,y: 600};
   var update = F2(function (input,model) {
      var newPlayer = A2($SpaceGame$Player.update,input,model.player);
   ...
   var init = function () {
      ...
      var starFieldGenerator = $SpaceGame$StarField.random(window);
      var generatedValue = A2($Utilities$Random.generate,starFieldGenerator,seed);
      return A2(Model,$SpaceGame$Player.init,generatedValue.value);
   }();
   var Input = F4(function (a,b,c,d) {    return {x: a,y: b,delta: c,time: d};});
   var timedInputSignal = function () {
      var delta = $Time.fps(35);
      var deltaTimestamp = $Time.timestamp(delta);
      var time = A2($Signal.map,function (deltaTimestamp) {    return {time: $Basics.fst(deltaTimestamp),delta: $Basics.snd(deltaTimestamp)};},deltaTimestamp);
      return A2($Signal.sampleOn,
      delta,
      A3($Signal.map2,F2(function (time,input) {    return A4(Input,input.x,input.y,time.delta,time.time);}),time,$Keyboard.arrows));
   }();
   var model = A3($Signal.foldp,update,init,timedInputSignal);
   var main = A2($Signal.map,view,model);
   return _elm.SpaceGame.Main.values = {_op: _op
                                       ,Input: Input
                                       ,Model: Model
                                       ,window: window
                                       ,init: init
                                       ,timedInputSignal: timedInputSignal
                                       ,model: model
                                       ,update: update
                                       ,view: view
                                       ,main: main};
