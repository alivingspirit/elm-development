...
Elm.GuessField = Elm.GuessField || {};
Elm.GuessField.make = function (_elm) {
   "use strict";
   _elm.GuessField = _elm.GuessField || {};
   if (_elm.GuessField.values) return _elm.GuessField.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var isAnswered = function (model) {
      var _p0 = model.guess;
      if (_p0.ctor === "GuessNumber") {
            return _U.eq(_p0._0,model.answer) ? true : false;
         } else {
            return false;
         }
   };
   var update = F2(function (action,model) {    if (isAnswered(model)) return model; else {    var _p1 = action;return _U.update(model,{guess: _p1._0});}});
   var GuessChanged = function (a) {    return {ctor: "GuessChanged",_0: a};};
   var Error = function (a) {    return {ctor: "Error",_0: a};};
   var GuessNumber = function (a) {    return {ctor: "GuessNumber",_0: a};};
   var init = function ($int) {    return {guess: GuessNumber(0),answer: $int};};
   var stringToAction = function (string) {
      var result = $String.toInt(string);
      var _p2 = result;
      if (_p2.ctor === "Ok") {
            return GuessChanged(GuessNumber(_p2._0));
         } else {
            return GuessChanged(Error(_p2._0));
         }
   };
   var makeMessage = F2(function (address,string) {    return A2($Signal.message,address,stringToAction(string));});
   var Model = F2(function (a,b) {    return {guess: a,answer: b};});
   _op["=>"] = F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};});
   var colorAttribute = function (model) {
      return isAnswered(model) ? _U.list([$Html$Attributes.style(_U.list([A2(_op["=>"],"background-color","green")]))]) : _U.list([]);
   };
   var view = F2(function (address,model) {
      var s = function () {
         var _p3 = model.guess;
         if (_p3.ctor === "GuessNumber") {
               var _p4 = _p3._0;
               return _U.cmp(_p4,model.answer) > 0 ? "Too High" : _U.cmp(_p4,model.answer) < 0 ? "Too Low" : "Correct";
            } else {
               return "";
            }
      }();
      ...
      _U.list([A2($Html.input,
              A2($Basics._op["++"],_U.list([A3($Html$Events.on,"input",$Html$Events.targetValue,makeMessage(address))]),colorAttribute(model)),
              _U.list([]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text(s)]))]));
   });
   return _elm.GuessField.values = {_op: _op
                                   ,Model: Model
                                   ,GuessNumber: GuessNumber
                                   ,Error: Error
                                   ,GuessChanged: GuessChanged
                                   ,init: init
                                   ,update: update
                                   ,view: view
...
Elm.Main.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $GuessField = Elm.GuessField.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var _op = {};
   var randomIntGenerator = A2($Random.$int,1,100);
   var Model = F3(function (a,b,c) {    return {gameState: a,guessField: b,seed: c};});
   var Finished = {ctor: "Finished"};
   var Started = {ctor: "Started"};
   var update = F2(function (_p0,model) {
      var _p1 = _p0;
      var _p2 = _p1._1;
      if (_p2.ctor === "GuessFieldAction") {
            var newGuessField = A2($GuessField.update,_p2._0,model.guessField);
            var newState = $GuessField.isAnswered(newGuessField) ? Finished : model.gameState;
            return _U.update(model,{guessField: newGuessField,gameState: newState});
         } else {
            var time = $Basics.round(_p1._0);
            var oldSeed = function () {
               var _p3 = model.seed;
               if (_p3.ctor === "Nothing") {
                     return $Random.initialSeed(time);
                  } else {
                     return _p3._0;
                  }
            }();
            var _p4 = A2($Random.generate,randomIntGenerator,oldSeed);
            var randomNumber = _p4._0;
            var newSeed = _p4._1;
            return _U.update(model,{guessField: $GuessField.init(randomNumber),gameState: Started,seed: $Maybe.Just(newSeed)});
         }
   });
   var NotStarted = {ctor: "NotStarted"};
   var init = function (_p5) {    var _p6 = _p5;return A3(Model,NotStarted,$GuessField.init(0),$Maybe.Nothing);};
   var GuessFieldAction = function (a) {    return {ctor: "GuessFieldAction",_0: a};};
   var NewGame = {ctor: "NewGame"};
   var mailbox = $Signal.mailbox(NewGame);
   var guessFieldAddress = A2($Signal.forwardTo,mailbox.address,GuessFieldAction);
   var model = A3($Signal.foldp,update,init({ctor: "_Tuple0"}),$Time.timestamp(mailbox.signal));
   var view = function (model) {
      var appView = function () {
         var _p7 = model.gameState;
         switch (_p7.ctor)
         {case "NotStarted": return A2($Html.button,_U.list([A2($Html$Events.onClick,mailbox.address,NewGame)]),_U.list([$Html.text("New Game")]));
            case "Finished": return A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,mailbox.address,NewGame)]),_U.list([$Html.text("New Game")]))
                      ,A2($GuessField.view,guessFieldAddress,model.guessField)]));
            default: return A2($GuessField.view,guessFieldAddress,model.guessField);}
      }();
      return A2($Html.div,_U.list([]),_U.list([appView]));
   };
   var main = A2($Signal.map,view,model);
   return _elm.Main.values = {_op: _op
                             ,NewGame: NewGame
                             ,GuessFieldAction: GuessFieldAction
                             ...
                             ,mailbox: mailbox
                             ,guessFieldAddress: guessFieldAddress
                             ,update: update
                             ,model: model
                             ,view: view
                             ,main: main};
